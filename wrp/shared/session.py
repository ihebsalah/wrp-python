...
class RequestResponder(Generic[ReceiveRequestT, SendResultT]):
    """Handles responding to WRP requests and manages request lifecycle.
...
class BaseSession(
    ...
    """
    Implements an WRP "session" on top of read/write streams, including features
    ...
    async def send_request(
        ...
        """
        Sends a request and wait for a response. Raises an WrpError if the
        ...
        try:
            ...
            if isinstance(response_or_error, JSONRPCError):
                raise WrpError(response_or_error.error)
            else:
