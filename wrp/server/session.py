# wrp/server/session.py
"""
ServerSession Module

This module provides the ServerSession class, which manages communication between the
server and client in the WRP (Workflow Runtime Protocol) framework. It is most commonly
used in WRP servers to interact with the client.
...
```
    ...
    async def handle_tool_call(ctx: RequestContext, arguments: dict[str, Any]) -> Any:
        ...
        if ctx.session.check_client_capability(
            WRPClientCapabilities(experimental={"advanced_tools": dict()})
        ):
...
be instantiated directly by users of the WRP framework.
"""
...
from wrp.shared.version import SUPPORTED_PROTOCOL_VERSIONS
...
class ServerSession(
    ...
    async def send_workflows_list_changed(self) -> None:
        """Send a workflows/list_changed notification (WRP)."""
        await self.send_notification(types.ServerNotification(types.WorkflowsListChangedNotification()))
