# wrp/client/streamable_http.py
"""
StreamableHTTP Client Transport Module

This module implements the StreamableHTTP transport for WRP clients,
...
WRP_SESSION_ID = "wrp-session-id"
WRP_PROTOCOL_VERSION = "wrp-protocol-version"
...
class StreamableHTTPTransport:
    ...
    def _prepare_request_headers(self, base_headers: dict[str, str]) -> dict[str, str]:
        ...
        if self.session_id:
            headers[WRP_SESSION_ID] = self.session_id
        if self.protocol_version:
            headers[WRP_PROTOCOL_VERSION] = self.protocol_version
        return headers
    ...
    def _maybe_extract_session_id_from_response(
        ...
        new_session_id = response.headers.get(WRP_SESSION_ID)
        if new_session_id:
...
async def streamablehttp_client(
    ...
    httpx_client_factory: WrpHttpClientFactory = create_wrp_http_client,
    auth: httpx.Auth | None = None,
) -> AsyncGenerator[
    tuple[
